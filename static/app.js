...
class SuperBizAgentApp {
    ...
    initializeElements() {
        ...
        // 知识图谱相关元素
        this.kgSidebarBtn = document.getElementById('kgSidebarBtn');
        this.kgModal = document.getElementById('kgModal');
        this.kgCloseBtn = document.getElementById('kgCloseBtn');
    ...
    bindEvents() {
        ...
        // 知识图谱按钮
        if (this.kgSidebarBtn) {
            this.kgSidebarBtn.addEventListener('click', () => this.openKGModal());
        }
        if (this.kgCloseBtn) {
            this.kgCloseBtn.addEventListener('click', () => this.closeKGModal());
        }
        if (this.kgModal) {
            this.kgModal.addEventListener('click', (e) => {
                if (e.target === this.kgModal) this.closeKGModal();
            });
        }
    ...
    async openKGModal() {
        if (this.kgModal) {
            this.kgModal.style.display = 'flex';
            document.body.style.overflow = 'hidden';
            await this.loadKGGraph();
        }
    }

    closeKGModal() {
        if (this.kgModal) {
            this.kgModal.style.display = 'none';
            document.body.style.overflow = '';
        }
    }

    async loadKGGraph(alertFilter = '') {
        try {
            const url = alertFilter
                ? `${this.apiBaseUrl}/kg/analyze/${encodeURIComponent(alertFilter)}`
                : `${this.apiBaseUrl}/kg/graph`;
            ...
            if (alertFilter) {
                // 显示单个告警分析 + 子图
                this.renderAlertAnalysis(result.data);
                // 同时加载完整图谱并高亮
                const fullResp = await fetch(`${this.apiBaseUrl}/kg/graph`);
                const fullResult = await fullResp.json();
                this.renderKGNetwork(fullResult.data, alertFilter);
            } else {
                this.renderKGNetwork(result.data, '');
                const analysisPanel = document.getElementById('kgAnalysis');
                if (analysisPanel) {
    ...
    renderKGNetwork(graphData, highlightAlert) {
        ...
        const nodes = new vis.DataSet(graphData.nodes.map(n => ({
            ...
            label: n.label,
            color: {
                background: highlightAlert && n.id === highlightAlert ? '#fff3e0' : n.color,
                border: highlightAlert && n.id === highlightAlert ? '#e65100' : n.color,
                highlight: { background: '#fff3e0', border: '#e65100' },
            },
            font: { color: '#fff', size: 12 },
            shape: n.type === 'alert' ? 'diamond' : n.type === 'root_cause' ? 'dot' : n.type === 'action' ? 'box' : 'triangle',
            size: n.type === 'alert' ? 25 : 18,
            borderWidth: highlightAlert && n.id === highlightAlert ? 3 : 1,
        })));
        ...
        const edges = new vis.DataSet(graphData.edges.map(e => ({
            ...
            to: e.to,
            label: e.label,
            color: { color: e.color, highlight: '#e65100' },
            dashes: e.dashes,
            arrows: 'to',
        ...
        if (this.kgNetwork) {
            this.kgNetwork.destroy();
        }
        this.kgNetwork = new vis.Network(container, { nodes, edges }, options);

        // 点击节点显示分析
        this.kgNetwork.on('click', async (params) => {
            if (params.nodes.length > 0) {
                const nodeId = params.nodes[0];
                const nodeData = graphData.nodes.find(n => n.id === nodeId);
                if (nodeData && nodeData.type === 'alert') {
                    this.kgAlertFilter.value = nodeId;
                    await this.filterKGGraph(nodeId);
                }
            }
        });
        ...
        if (highlightAlert) {
            setTimeout(() => {
                this.kgNetwork.focus(highlightAlert, { scale: 1.2, animation: true });
            }, 500);
        }
    }

    renderAlertAnalysis(analysis) {
