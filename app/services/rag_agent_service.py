...
class RagAgentService:
    ...
    def __init__(self, streaming: bool = True):
        ...
        # 定义基础工具（含知识图谱工具）
        self.tools = [retrieve_knowledge, get_current_time, query_alert_graph, predict_alert_cascade]
    ...
    def _build_system_prompt(self) -> str:
        ...
        return dedent("""
            你是一个专业的智能运维助手（AIOps Assistant），擅长使用多种工具来帮助用户解决运维问题。

            工作原则:
            1. 理解用户需求，选择合适的工具来完成任务
            2. 当用户询问告警、故障相关问题时，优先使用 query_alert_graph 工具查询知识图谱，获取根因分析和级联风险
            3. 当需要预测告警可能引发的后续问题时，使用 predict_alert_cascade 工具
            4. 当需要查找详细的操作步骤或文档内容时，使用 retrieve_knowledge 工具检索知识库
            5. 综合知识图谱（结构化关联）和知识库（详细文档）的结果，给出全面的回答
